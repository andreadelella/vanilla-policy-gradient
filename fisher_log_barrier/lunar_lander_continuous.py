"""Train fixed or gradient-clipped Fisher regularization on LunarLanderContinuous."""
...
class LunarLanderContinuousConfig:
    seed: int = 101
    hidden_sizes: tuple[int, ...] = (16, 16)
    updates: int = 300
    reward_trajectory_count: int = 32
    fisher_trajectory_count: int = 512
    workers: int = 32
    horizon: int = 1000
    gamma: float = 0.99
    learning_rate: float = 3e-4
    fisher_beta: float = 1e-6
    fisher_mu: float = 0.0
...
def _make_env(config: LunarLanderContinuousConfig):
    def thunk():
...
def _build_policy(config: LunarLanderContinuousConfig) -> GaussianPolicy:
