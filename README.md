...
| `training_rewards_ci.png` | analysis notebook | Mean training return ± 95% CI across seeds, generated from `.npz` or legacy `.npy` rewards in `notebooks/ci_plots.ipynb` (no longer produced at train time). |
| `best_seed_comparison.png` | analysis notebook | Best-performing seed of each run overlaid, generated by `notebooks/comparison.ipynb`. |
...
Analysis is done in `notebooks/`, each pointed at one or more run directories (label → path):

- `comparison.ipynb` — compares several runs (algorithms or network sizes): mean ± 95% CI
  across seeds (`comparison.png`), plus a best-seed-per-run overlay (`best_seed_comparison.png`).
- `ci_plots.ipynb` — auto-detects `training_rewards.npz` or legacy
  `training_rewards.npy`, normalizes one- and two-dimensional arrays, preserves
  NPZ seed IDs, and plots a single run's mean ± 95% CI band
  (`training_rewards_ci.png`).

Train each variant into its own directory, then point the notebook at them:
