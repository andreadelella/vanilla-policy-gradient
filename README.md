...
| `training_rewards_ci.png` | analysis notebook | Mean training return ± 95% CI across seeds, generated from the saved `.npz` in `notebooks/ci_plots.ipynb` (no longer produced at train time). |
...
- `ci_plots.ipynb` — plots a single run's mean ± 95% CI band across seeds
