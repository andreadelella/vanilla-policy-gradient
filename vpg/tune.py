...
def _objective(trial: optuna.Trial, args, algorithm: str) -> float:
    ...
    for seed_idx in range(args.n_tuning_seeds):
        ...
        # Each trial gets its own temporary folder so checkpoints and logs do not collide.
        ...
        try:
            ...
            # run_single_training returns the final policy, but the project saves the best
            # training-return checkpoint as policy/best.pt. We evaluate that same file
            # so the score reflects what a real tuned run would actually use.
            best_checkpoint = os.path.join(trial_dir, "policy", "best.pt")
...
def run_search(args) -> dict:
    ...
    # A value of None means "run the full grid" for programmatic callers.
    # The CLI default uses a smaller screening pass, so we keep that behavior here.
